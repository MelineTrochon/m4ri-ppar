...
static inline int _mzd_gauss_submatrix_full(mzd_t *A, rci_t r, rci_t c, rci_t end_row, int k) {
  ...
  for (j = c; j < c + k; ++j) {
    ...
    for (rci_t i = start_row; i < end_row; ++i) {
      ...
      if (tmp) {
        for (int l = 0; l < j - c; ++l)
          if (__M4RI_GET_BIT(tmp, l)) mzd_row_add_offset(A, i, r + l, c + l);
        ...
        if (mzd_read_bit(A, i, j)) {
          ...
          for (rci_t l = r; l < start_row; ++l) {
            if (mzd_read_bit(A, l, j)) { mzd_row_add_offset(A, l, start_row, j); }
          }
          ++start_row;
          found = 1;
          break;
...
static inline int _mzd_gauss_submatrix(mzd_t *A, rci_t r, rci_t c, rci_t end_row, int k) {
  ...
  for (j = c; j < c + k; ++j) {
    ...
    for (rci_t i = start_row; i < end_row; ++i) {
      /* first we need to clear the first columns */
      for (int l = 0; l < j - c; ++l)
        if (mzd_read_bit(A, i, c + l)) mzd_row_add_offset(A, i, r + l, c + l);
...
static inline void _mzd_copy_back_rows(mzd_t *A, mzd_t const *U, rci_t r, rci_t c, int k) {
  ...
  for (int i = 0; i < k; ++i) {
    word const * src = mzd_row_const(U, i) + startblock;
    word * dst       = mzd_row(A, r + i) + startblock;
    for (wi_t j = 0; j < width; ++j) { dst[j] = src[j]; }
  }
  __M4RI_DD_MZD(A);
}
...
void mzd_make_table(mzd_t const *M, rci_t r, rci_t c, int k, mzd_t *T, rci_t *L) {
  ...
  word const mask_begin =
      (M->width - homeblock != 1) ? pure_mask_begin : pure_mask_begin & mask_end;
  wi_t const wide = M->width - homeblock;

  int const twokay = __M4RI_TWOPOW(k);
  L[0]             = 0;
  for (rci_t i = 1; i < twokay; ++i) {
    word *ti  = mzd_row(T, i) + homeblock;
    word *ti1 = mzd_row(T, i - 1) + homeblock;

    rci_t const rowneeded = r + m4ri_codebook[k]->inc[i - 1];
    int const id          = m4ri_codebook[k]->ord[i];
    L[id]                 = i;

    if (rowneeded >= M->nrows) continue;

    word const *m = mzd_row_const(M, rowneeded) + homeblock;
...
void mzd_process_rows(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
                      mzd_t const *T, rci_t const *L) {
  ...
  if (k == 1) {
    ...
    for (r = startrow; r + 2 <= stoprow; r += 2) {
      word const b0 = mzd_row(M, r + 0)[block] & bm;
      word const b1 = mzd_row(M, r + 1)[block] & bm;

      word *m0 = mzd_row(M, r + 0) + block;
      word *m1 = mzd_row(M, r + 1) + block;
      word const *t  = mzd_row_const(T, 1) + block;
    ...
    for (; r < stoprow; ++r) {
      ...
      word *m0 = mzd_row(M, r) + block;
      word const *t0 = mzd_row_const(T, x0) + block;
  ...
  for (r = startrow; r + 2 <= stoprow; r += 2) {
    ...
    word *m1 = mzd_row(M, r + 1) + block;
    word const *t1 = mzd_row_const(T, x1) + block;
...
void mzd_process_rows2(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
  ...
  for (r = startrow; r < stoprow; ++r) {
    ...
    if ((x0 | x1) == 0)  // x0 == 0 && x1 == 0
      continue;
    word *m0 = mzd_row(M, r) + blocknum;
    word const *t[2];
    t[0] = mzd_row_const(T0, x0) + blocknum;
    t[1] = mzd_row_const(T1, x1) + blocknum;
...
void mzd_process_rows3(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
  ...
  for (r = startrow; r < stoprow; ++r) {
    ...
    t[0] =  mzd_row_const(T0, x0) + blocknum;
    t[1] =  mzd_row_const(T1, x1) + blocknum;
    t[2] =  mzd_row_const(T2, x2) + blocknum;
...
void mzd_process_rows4(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
  ...
  for (r = startrow; r < stoprow; ++r) {
    ...
    t[1] =  mzd_row_const(T1, x1) + blocknum;
    t[2] =  mzd_row_const(T2, x2) + blocknum;
    t[3] =  mzd_row_const(T3, x3) + blocknum;
...
void mzd_process_rows5(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
  ...
  for (r = startrow; r < stoprow; ++r) {
    ...
    t[2] =  mzd_row_const(T2, x2) + blocknum;
    t[3] =  mzd_row_const(T3, x3) + blocknum;
    t[4] =  mzd_row_const(T4, x4) + blocknum;
...
void mzd_process_rows6(mzd_t *M, rci_t startrow, rci_t stoprow, rci_t startcol, int k,
  ...
  for (r = startrow; r < stoprow; ++r) {
    ...
    t[3] =  mzd_row_const(T3, x3) + blocknum;
    t[4] =  mzd_row_const(T4, x4) + blocknum;
    t[5] =  mzd_row_const(T5, x5) + blocknum;
...
rci_t _mzd_echelonize_m4ri(mzd_t *A, int const full, int k, int heuristic, double const threshold) {
  ...
  if (k == 0) {
    ...
    if (k >= 7) k = 7;
    if (0.75 * __M4RI_TWOPOW(k) * ncols > __M4RI_CPU_L3_CACHE / 2.0) k -= 1;
  }
  int kk = 6 * k;
  ...
  mzd_t *T0 = mzd_init_window(T, 0 * __M4RI_TWOPOW(k), align_offset, 1 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  mzd_t *T1 = mzd_init_window(T, 1 * __M4RI_TWOPOW(k), align_offset, 2 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  mzd_t *T2 = mzd_init_window(T, 2 * __M4RI_TWOPOW(k), align_offset, 3 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  mzd_t *T3 = mzd_init_window(T, 3 * __M4RI_TWOPOW(k), align_offset, 4 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  mzd_t *T4 = mzd_init_window(T, 4 * __M4RI_TWOPOW(k), align_offset, 5 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  mzd_t *T5 = mzd_init_window(T, 5 * __M4RI_TWOPOW(k), align_offset, 6 * __M4RI_TWOPOW(k),
                              ncols + align_offset);
  ...
  while (c < ncols) {
    if (heuristic && c > (last_check + 256)) {
      ...
      if (c < ncols && r < A->nrows && _mzd_density(A, 32, r, c) >= threshold) {
        ...
        if (!full) {
          ...
          if (r > 0) _mzd_top_echelonize_m4ri(A, 0, r, c, r);
          r += r2;
        }
        mzd_free(Abar);
        break;
    ...
    if (kbar > 5 * k) {
      ...
      if (kbar == kk)
        mzd_process_rows6(A, r + kbar, A->nrows, c, kbar, T0, L0, T1, L1, T2, L2, T3, L3, T4, L4,
                          T5, L5);
      if (full) mzd_process_rows6(A, 0, r, c, kbar, T0, L0, T1, L1, T2, L2, T3, L3, T4, L4, T5, L5);
      ...
      if (full) mzd_process_rows5(A, 0, r, c, kbar, T0, L0, T1, L1, T2, L2, T3, L3, T4, L4);
      ...
      if (full) mzd_process_rows4(A, 0, r, c, kbar, T0, L0, T1, L1, T2, L2, T3, L3);
      ...
      if (kbar == kk) mzd_process_rows3(A, r + kbar, A->nrows, c, kbar, T0, L0, T1, L1, T2, L2);
      if (full) mzd_process_rows3(A, 0, r, c, kbar, T0, L0, T1, L1, T2, L2);
      ...
      if (kbar == kk) mzd_process_rows2(A, r + kbar, A->nrows, c, kbar, T0, L0, T1, L1);
      if (full) mzd_process_rows2(A, 0, r, c, kbar, T0, L0, T1, L1);

    } else if (kbar > 0) {
      if (full || kbar == kk) { mzd_make_table(A, r, c, kbar, T0, L0); }
      if (kbar == kk) mzd_process_rows(A, r + kbar, A->nrows, c, kbar, T0, L0);
      if (full) mzd_process_rows(A, 0, r, c, kbar, T0, L0);
    }

    if (!full) { _mzd_copy_back_rows(A, U, r, c, kbar); }
...
rci_t _mzd_top_echelonize_m4ri(mzd_t *A, int k, rci_t r, rci_t c, rci_t max_r) {
  ...
  if (k == 0) {
    ...
    if (k >= 7) k = 7;
    if (0.75 * __M4RI_TWOPOW(k) * A->ncols > __M4RI_CPU_L3_CACHE / 2.0) k -= 1;
  }
  int kk = 6 * k;
  ...
  while (c < ncols) {
    ...
    if (kbar > 5 * k) {
      ...
      mzd_make_table(A, r + ka + kb + kc + kd, c, ke, T4, L4);
      mzd_make_table(A, r + ka + kb + kc + kd + ke, c, kf, T5, L5);
      mzd_process_rows6(A, 0, MIN(r, max_r), c, kbar, T0, L0, T1, L1, T2, L2, T3, L3, T4, L4, T5,
                        L5);
...
void mzd_top_echelonize_m4ri(mzd_t *M, int k) { _mzd_top_echelonize_m4ri(M, k, 0, 0, M->nrows); }
...
mzd_t *mzd_addmul_m4rm(mzd_t *C, mzd_t const *A, mzd_t const *B, int k) {
  ...
  if (C->ncols == 0 || C->nrows == 0) return C;
  ...
  if (C == NULL) {
    ...
    if (C->nrows != a || C->ncols != c) m4ri_die("mzd_mul_m4rm: C has wrong dimensions.\n");
  }
...
mzd_t *_mzd_mul_m4rm(mzd_t *C, mzd_t const *A, mzd_t const *B, int k, int clear) {
  ...
  if (k == 0) {
    ...
    if ((__M4RI_CPU_L2_CACHE - 64 * __M4RI_TWOPOW(k) * B->width) >
        (64 * __M4RI_TWOPOW(k + 1) * B->width - __M4RI_CPU_L2_CACHE))
      k++;
  ...
  for (rci_t giantstep = 0; giantstep < a_nr; giantstep += blocksize) {
    for (rci_t i = 0; i < end; ++i) {
      ...
      for (rci_t j = giantstep; j < blockend; j++) {
        ...
        case 8: t[7] = mzd_row_const(T[7], L[7][(a >> 7 * k) & bm]);
        case 7: t[6] = mzd_row_const(T[6], L[6][(a >> 6 * k) & bm]);
        case 6: t[5] = mzd_row_const(T[5], L[5][(a >> 5 * k) & bm]);
        case 5: t[4] = mzd_row_const(T[4], L[4][(a >> 4 * k) & bm]);
        case 4: t[3] = mzd_row_const(T[3], L[3][(a >> 3 * k) & bm]);
        case 3: t[2] = mzd_row_const(T[2], L[2][(a >> 2 * k) & bm]);
        case 2: t[1] = mzd_row_const(T[1], L[1][(a >> 1 * k) & bm]);
        case 1: t[0] = mzd_row_const(T[0], L[0][(a >> 0 * k) & bm]); break;
        default: m4ri_die("__M4RI_M4RM_NTABLES must be <= 8 but got %d", __M4RI_M4RM_NTABLES);
        }
        ...
        case 1: _mzd_combine(c, t[0], wide); break;
        default: m4ri_die("__M4RI_M4RM_NTABLES must be <= 8 but got %d", __M4RI_M4RM_NTABLES);
        }
      }
    }
  ...
  if (a_nc % kk) {
    ...
    for (i = kk / k * end; i < a_nc / k; ++i) {
      ...
      for (rci_t j = 0; j < a_nr; ++j) {
        ...
        word *c = mzd_row(C, j);
        t[0] = mzd_row_const(T[0], x[0]);
        for (wi_t ii = 0; ii < wide; ++ii) { c[ii] ^= t[0][ii]; }
      }
    }
